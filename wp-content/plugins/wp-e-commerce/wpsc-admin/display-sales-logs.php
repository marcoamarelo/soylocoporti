		...
		?>
			<div id='post-body' class='has-sidebar' style='width:95%;'>
				...
				<div id='wpsc_shipping_details_box'>	
					<h3><?php _e('Detalhes de postagem'); ?></h3>
					<p>
					<?php 
					
					$shippinginfo = wpsc_getall_shippinginfo();

					foreach ($shippinginfo as $info) {
						
						echo $info['name'].": ".$info['value']."<br/>";
						
					}
					
					?>
					</p>
					<strong><?php _e('Opções de postagem'); ?></strong>
					<p>
					
					<?php _e('Transportadora:'); ?> <?php echo wpsc_display_purchlog_shipping_method(); ?><br />
					<?php _e('Opção:'); ?> <?php echo wpsc_display_purchlog_shipping_option(); ?><br />
					<?php if(wpsc_purchlogs_has_tracking()) : ?>
						<?php _e('Código de rastreamento:'); ?> <?php echo wpsc_purchlogitem_trackid(); ?><br />
						<?php _e('Situação:'); ?> <?php echo wpsc_purchlogitem_trackstatus(); ?><br />
						<?php _e('Histórico:'); ?> <?php echo wpsc_purchlogitem_trackhistory(); ?>
					<?php endif; ?>
					</p>
				</div>
				<?php } ?>
				<div id='wpsc_billing_details_box'>
					<h3><?php _e('Detalhes de cobrança'); ?></h3>
					<p><strong><?php _e('Data da compra:'); ?> </strong><?php echo wpsc_purchaselog_details_date(); ?> </p>
					<p><strong><?php _e('Número do pedido:'); ?> </strong><?php echo wpsc_purchaselog_details_purchnumber(); ?> </p>
					<p>
					<strong>Dados do comprador</strong><br/>
					<?php 
					
					$billinginfo = wpsc_getall_billinginfo();
					...
					<p><strong><?php _e('Método de pagamento:'); ?> </strong><?php echo wpsc_display_purchlog_paymentmethod(); ?></p>
				...
				<div id='wpsc_items_ordered'>
					<br />
					<h3><?php _e('Relação de itens do pedido'); ?></h3>
...
<?php
 ...
 function get_purchaselogs_content(){
 	...
 	<tr>
 		...
 		<td><strong><a href='<?php echo htmlentities(add_query_arg('purchaselog_id', wpsc_the_purch_item_id())) ; ?>'><?php echo wpsc_the_purch_item_id(); ?></a></strong></td> <!--ID -->
 		...
 		<td><a href='<?php echo htmlentities(add_query_arg('purchaselog_id', wpsc_the_purch_item_id())) ; ?>'><?php echo wpsc_the_purch_item_details();?> Item(s)</a></td><!-- Details -->
 		<td>
 		...
 		<td>
 			<a class='wpsc_show_trackingid' title='<?php echo wpsc_the_purch_item_id(); ?>' href=''>+ rastreamento</a>
 		</td>
 	</tr>
 	<tr class='log<?php echo wpsc_the_purch_item_id(); ?> wpsc_trackingid_row'>
 		...
 		<td class='wpsc_trackingid_row' >
 			<label for='wpsc_trackingid<?php echo wpsc_the_purch_item_id(); ?>'>ID:</label>
 		</td>
 		<td class='wpsc_trackingid_row' colspan='2'>
 			...
 			<input type='submit' name='submit' class='button' value='Cadastrar código de rastreamento' />
 			<input type='hidden' name='addtrackid' value='1'/>
 		</td>
 		<td colspan='4'>
 			<a href='' title='<?php echo wpsc_the_purch_item_id(); ?>' class='sendTrackingEmail'>Enviar uma mensagem personalizada</a>
 		</td>
 	</tr>
