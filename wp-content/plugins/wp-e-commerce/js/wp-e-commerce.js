...
jQuery(document).ready(function () {
	...
	var cepcalculo = jQuery('#zipcode').val();
	if (cepcalculo != 'Seu CEP') {
		jQuery("input[title='shippingpostcode']").val(cepcalculo);
		jQuery("input[title='shippingpostcode']").attr("readonly", true);
		
		jQuery("input[title='shippingpostcode']").click(function(e) {
				alert("Este CEP é o mesmo que o usado para o cálculo do seu frete. Para alterá-lo, por favor, utilize o botão de cálculo de frete.");
     ...
     jQuery("#shippingSameBilling").click(function(){
        ...
        jQuery("#shippingSameBilling").livequery(function(){
        	...
        	if(jQuery(this).is(":checked")){    
	            ...
	            var addrnumber = jQuery("input[title='billingaddressnumber']").val();
	            var addrcomplemento = jQuery("input[title='billingaddresscomplemento']").val();
	            var addrbairro = jQuery("input[title='billingaddressbairro']").val();
				var city = jQuery("input[title='billingcity']").val(); 
				var addrstate = jQuery("input[title='billingstate']").val();
				...
				if (pcode != jQuery('#zipcode').val()) {
					alert("O CEP utilizado para cálculo do frete é diferente do informado para entrega. Para alterá-lo, por favor, utilize o botão de cálculo de frete.");
					jQuery('#zipcode').focus();
				...
				jQuery("input[title='shippingaddressnumber']").val(addrnumber);
		        jQuery("input[title='shippingaddresscomplemento']").val(addrcomplemento);
		        jQuery("input[title='shippingaddressbairro']").val(addrbairro);
				jQuery("input[title='shippingcity']").val(city);
				jQuery("input[title='shippingstate']").val(addrstate);
