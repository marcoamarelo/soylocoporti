		...
		if(($cart != null) && ($errorcode == 0)) {
			...
			//	do_action('wpsc_confirm_checkout', $purchase_log['id']);
			  ...
			  $total_shipping += $purchase_log['base_shipping'];
				...
				// echo $total;
				// $message.= "\n\r";
				$product_list.= "O número do seu pedido é: ".$purchase_log['id']."\n\r";
				...
				$product_list_html.= "O número do seu pedido é: ".$purchase_log['id']."\n\n\r";
				if($purchase_log['discount_value'] > 0) {
        ...
        //$message_html = str_replace('%order_status%',get_option('blogname'),$message_html);
				...
				if(($email != '') && ($purchase_log['email_sent'] != 1)) {
					...
					if($purchase_log['processed'] < 2) {
						...
						wp_mail($email, __('Order Pending: Payment Required', 'wpsc'), $message, "From: ".get_option('blogname')." <".get_option('admin_email').">");
					} else {
						wp_mail($email, __('Purchase Receipt', 'wpsc'), $message, "From: ".get_option('blogname')." <".get_option('admin_email').">");
					}
				}
...
*/
				...
				if((get_option('purch_log_email') != null) && ($purchase_log['email_sent'] != 1)) {
					...
					wp_mail(get_option('purch_log_email'), __('Purchase Report', 'wpsc'), $report, "From: ".get_option('blogname')." <".get_option('admin_email').">");
