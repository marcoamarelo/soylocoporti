...
// Função para capturar atualização do estado da abertura do carrinho
if($_POST['set_slider'] == 'true') {
...
function wpsc_get_cart() {
  global $wpdb, $wpsc_cart, $wpsc_theme_path;
	...
	if(get_option('show_sliding_cart') == 1)	{
		if((wpsc_cart_item_count() > 0 || count($cart_messages) > 0) && $_SESSION['slider_state'] != "0" ) {
			$_SESSION['slider_state'] = 1;
			echo "
				jQuery('#sliding_cart').slideDown('fast',function(){
					jQuery('#fancy_collapser').attr('src', (WPSC_URL+'/images/minus.png'));
...
function atualizaParcelamento() {
	...
	for ($i=1;$i<=4;$i++) {
		$optionList .= "<option value='".$i."'>".$i."x R$".sprintf("%0.2f",(round($total/$i,2)))." (sem juros)</option>";		
	}	

	echo "jQuery('#card_parcelamento').html(\"".$optionList."\")";
} 
