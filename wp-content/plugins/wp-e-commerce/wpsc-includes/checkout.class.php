...
function wpsc_has_shipping_form(){
	...
	if((in_array('shippingcountry', $unique_name_list) || get_option('national_vendor') == '1') && (get_option('use_billing_unless_is_shipping') != '1')){
		return true;
	}else{
		return false;
	}
...
function wpsc_shipping_country_list($shippingdetails = false) {
	...
	// Check if is a national vendor. If no, display the country field.
	if (get_option('national_vendor')=="0")
	{
		...
		$output .= "<input type='hidden' name='country' value='$selected_country' />";
	}
	...
	if($uses_zipcode == true) {
		$output .= " <input type='text' style='color:".$color.";' onclick='if (this.value==\"Seu CEP\") {this.value=\"\";this.style.color=\"#000\";}' onblur='if (this.value==\"\") {this.style.color=\"#999\"; this.value=\"Seu CEP\"; }' value='".$zipvalue."' size='10' name='zipcode' id='zipcode'>";
	}
	return $output;
}
...
function wpsc_gateway_form_fields() {
	...
	if($wpsc_gateway->gateway['internalname'] == 'paypal_pro'){
		$output = sprintf($gateway_checkout_form_fields[$wpsc_gateway->gateway['internalname']] ,
							wpsc_the_checkout_CC_validation_class(),$_SESSION['wpsc_gateway_error_messages']['card_number'],
	...
	/*
	if($wpsc_gateway->gateway['internalname'] == 'cielo'){
		$output = sprintf($gateway_checkout_form_fields[$wpsc_gateway->gateway['internalname']],
