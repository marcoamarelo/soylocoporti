...
<table class="productcart">
	...
	<?php  //this displays the confirm your order html	?>
		...
		<tr class="product_row">
			...
			<td><span class="pricedisplay"><?php echo str_replace( '.', ',', wpsc_cart_item_price() ); ?></span></td>
	...
	<?php if(wpsc_uses_shipping()  && wpsc_has_shipping_form()): ?>
		...
		<table class="productcart">		
			...
			<?php if (!wpsc_have_shipping_quote()) : // No valid shipping quotes ?>
				<?php if (($_SESSION['wpsc_zipcode'] == '') || ($_SESSION['wpsc_zipcode'] == 'Seu CEP')) : // No valid shipping quotes ?>
			...
			<tr>
				<td colspan='5'>
					<form name='change_country' id='change_country' action='' method='post'>
						...
						<input type='submit' name='wpsc_submit_zipcode' value='Calcular frete' />
					</form>
				</td>
			</tr>
			...
			<?php if(wpsc_is_shipping_details()) : ?>
					<tr>
						<td colspan ='2'>
							...
							<label for='shippingSameBilling'>Usar para entrega o mesmo endereço que o de cobrança</label>
		...
		<tr>
			...
			<?php  //this HTML displays activated payment gateways?>
				...
				<?php if(wpsc_gateway_count() > 1): // if we have more than one gateway enabled, offer the user a choice ?>
					...
					<?php while (wpsc_have_gateways()) : wpsc_the_gateway(); ?>
						<div class="custom_gateway <?php echo wpsc_gateway_internal_name();?>">
							...
							<?php }else{ ?>
								<label><input type="radio" value="<?php echo wpsc_gateway_internal_name();?>" <?php echo wpsc_gateway_is_checked(); ?> name="custom_gateway" class="custom_gateway <?php echo wpsc_gateway_internal_name();?>"/><?php echo wpsc_gateway_name();?></label>
							<?php } ?>
				...
				<?php else: // otherwise, there is no choice, stick in a hidden form ?>
					<?php while (wpsc_have_gateways()) : wpsc_the_gateway(); ?>
						<input name='custom_gateway <?php echo wpsc_gateway_internal_name();?>' value='<?php echo wpsc_gateway_internal_name();?>' type='hidden' />
