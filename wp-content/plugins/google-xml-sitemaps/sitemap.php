...
/*
 Info for WordPress:
 ==============================================================================
 Plugin Name: Google XML Sitemaps with Multisite support
 Plugin URI: http://blog.mariokostelac.com/google-xml-sitemaps-multisite/
 Description: Improved Google XML Sitemaps plugin with Multisite Support
 Version: 4.0.2
...
class GoogleSitemapGeneratorLoader {
	...
	function Enable() {
		...
		//make rule for sitemap.xml
		//add_filter('rewrite_rules_array', array('GoogleSitemapGeneratorLoader', 'RewriteRule'));
	...
	// Adding a new rule
	function RewriteRule($rules)
	{
		$options = get_option( 'sm_options' );
		
		$newrules = array();
		$newrules['sitemap\.xml$'] = 'index.php?p=183';	
...
register_uninstall_hook(__FILE__, 'sm_uninstall');

if ( ! function_exists( 'sm_uninstall' ) ) {
	function gs_uninstall() {
